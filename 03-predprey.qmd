...
\begin{subequations}
\begin{align}
  \frac{dx}{dt} &= f(x,y) = x(1 - y),              \label{LVrescaledx}\\
  \frac{dy}{dt} &= g(x,y) = \mu y(x -  1),      \label{LVrescaledy}
\end{align}
\label{LVrescaled}
\end{subequations}
...
We refer to the set of points $(x,y)$ with $x\ge0$ and $y\ge0$ as our *state space*.  Geometrically,  Equations \eqref{LVrescaled} specify a vector field on the state space, and solutions of the equations are curves which are tangent to these vectors at each point.  A point $(x,y)$ for which $f(x,y)=g(x,y)=0$ is an equilibrium solution:  these are constant solutions satisfying the differential equations.
...
![Phase portrait](figures/figure-nullclines){#fig-nullclines}

Another trick that often provides insight is to consider the solutions curves as graphs of a function $y(x)$.  Since, under reasonable conditions, $dy/dx = (dy/dt)/(dx/dt)$, combining the two equations leads to a differential equation for $y(x)$.
\begin{equation}
  \frac{dy}{dx} = \frac{\mu y(x -  1)}{x(1 - y)}.
  \label{LVyofx}
\end{equation}
This can be integrated as
\begin{equation}
\Psi(x,y) = \mu (x - \log x) + y - \log y.
  \label{LVPsi}
\end{equation}
Solutions of \eqref{LVyofx} are level curves of $\Psi$.    Since $\Psi$ is concave in the positive quadrant, these will be closed curves, or periodic orbits, surrounding the interior equilibrium.

Now let $T$ be the period of oscillation of a particular solution to the model.  From \eqref{LVrescaled} we have
\begin{align*} 
...
![Nullclines for the Rosenzweig-MacAuther model](figures/figure-nullcline-2){#fig-nullcline-2}
...
\citet{turchin2003} provides an excellent discussion of the various responses of predators to changes in prey populations.    Consider a general predation model of the form
\begin{subequations}
\begin{align}
...
\citet{Holling1959} introduced a simple classification of functional responses of predators into type I, II, or III (see @fig-hollingIII).  Mathematically, type I and II are similar,  and represent a saturating response.   


![Holling's Classic functional response curves](figures/figure-hollingIII){#fig-hollingIII}
...
\begin{subequations}
\begin{align}
  \frac{dx}{dt} &= f(x,y), \label{planar ode x}\\
  \frac{dy}{dt} &= g(x,y),  \label{planar ode y}
\end{align}
  \label{planar ode}
\end{subequations}
...
An equilibrium solution of Equation \ref{planar ode} is a constant solution $(x(t),y(t)$ satisfying $f(x(t),y(t)) = g(x(t),y(t)) = 0$.  This can be visualized as a point in state-space.

Geometrically,  Equations \ref{planar ode} specify a vector field on the state space.  A solution passing through some point $(x,y)$ at time $t$ can be viewed as a curve tangent to the vector $(f(x,y),g(x,y))$.
...
With a few more theorems on the uniqueness of solutions and linear combinations of solutions that can be found in \citet{Logan2006ode} we can distill the results down to the diagram of @fig-trace-determinant-diagram.


![Stability of equilibria based on the trace and determinant of the Jacobian matrix](figures/figure-stability-diagram){#fig-trace-determinant-diagram}
